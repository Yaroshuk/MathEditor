...
import {
    insertText as plainInsertText,
    removeText as plainRemoveText,
    replaceText as plainReplaceText,
    mdInsertText,
    mdRemoveText,
    mdReplaceText,
    mdCutText,
...
import {
    createEmojiUpdatePayload,
    isCustomLink,
    tokenForPos,
} from "../formatted-string/utils";
...
export function toggleFormat(
    ...
    if (source) {
        const update: TokenFormatUpdate =
            source.format & format ? { remove: format } : { add: format };
...
export function applyFormatFromFragment(
    ...
    fragment.forEach((token) => {
        ...
        if (token.emoji?.length) {
            const payload = createEmojiUpdatePayload(
                token.emoji,
                offset,
                token.value
            );
