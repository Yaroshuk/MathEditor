...
export default class Editor {
  ...
  private onCompositionEnd = () => {
    if (this.composition) {
      ...
      this.updateModel(this.composition, DiffActionType.Compose, range);
  ...
  dispose(): void {
    ...
    this.element.removeEventListener(
      'input',
      this.onInput as (evt: Event) => void
    );
  ...
  replaceText(from: number, to: number, text: string): Model {
    ...
    const result = this.updateModel(nextModel, DiffActionType.Replace, [
      from,
      to,
    ]);
