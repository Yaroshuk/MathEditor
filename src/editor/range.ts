...
export function setDOMRange(range: Range): Range | void {
  ...
  try {
    if (sel.rangeCount) {
      ...
      const startBound = curRange.compareBoundaryPoints(
...
export function rangeToLocation(root: HTMLElement, range: Range): TextRange {
  ...
  const from = rangeBoundToLocation(
    root,
  ...
  const to = collapsed
    ? from
    : rangeBoundToLocation(root, range.endContainer, range.endOffset);
  return [from, to];
}
 ...
 */
export function rangeBoundToLocation(
  root: HTMLElement,
  bound: Node,
  pos: number
): number {
...
function sumNodesLength(root: HTMLElement, bound: Node, pos: number): number {
  ...
  result += isText(bound) ? pos : getFragmentLength(bound, pos);
 ...
 * для узла модели
 */
export function locationToRangeBound(
  root: HTMLElement,
  pos: number
): RangeBound {
  for (let i = 0; i < root.childNodes.length; i++) {
...
function getLineLength(line: Element): number {
  return (
    getLineBlockLength(line) + getFragmentLength(line, line.childNodes.length)
  );
}
...
function createWalker(elem: HTMLElement): TreeWalker {
  return elem.ownerDocument.createTreeWalker(
    elem,
