...
export default class ParserState {
  ...
  consumeLog(match: number | MatchFn): boolean {
    const ch = this.peek()!;
    const ok = typeof match === 'function' ? match(ch) : ch === match;
    console.log('consumeLog', { ch, ok }, match, this.string);
    if (ok) {
      this.inc(ch);
    }
  ...
  atFormula(): boolean {
    const lastToken = last(this.tokens);
    return lastToken?.type === TokenType.Formula;
  }
