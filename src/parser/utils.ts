...
export function isWhitespace(ch: number): boolean {
  return ch === Codes.Space || ch === Codes.NBSP || ch === Codes.Tab;
}
...
export function isBound(ch?: number): boolean {
  return (
    ch === undefined || ch !== ch /* NaN */ || isNewLine(ch) || isWhitespace(ch)
  );
}
...
export function toCode(str: string, ignoreCase?: boolean): number[] {
  ...
  for (let i = 0; i < str.length; i++) {
    result.push(
      ignoreCase ? asciiToUpper(str.codePointAt(i)!) : str.codePointAt(i)!
    );
  }
